...
public class EmailSpreadUtil {
	...
	public static String DEFAULT_CAMPAIGN_NAME = "tomtop";

	public boolean send(String from, String toEmail, String campaignName,
			String title, String content, String fromName) {
		try {
		...
		campaignName = StringUtils.isEmpty(campaignName) ? title : campaignName;
		fromName = StringUtils.isEmpty(fromName) ? DEFAULT_CAMPAIGN_NAME
				: fromName;
	...
	public boolean send(String from, String toEmail, String title,
		...
		boolean result = this.send(from, toEmail, "", title, content, fromName);
		if (false == result) {
