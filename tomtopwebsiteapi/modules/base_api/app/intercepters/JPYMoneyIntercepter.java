...
public class JPYMoneyIntercepter implements IMoneyIntercepter {
	...
	private static final String COOKIE_CURRENCY = "TT_CURR";
	...
	public String intercept(Double money, WebContext webCtx) {
		...
		if (webCtx != null && webCtx.getCurrencyCode() != null) {
			...
			String curr = httpCtx.request().getQueryString("currency");
			if (null != curr && curr.length() > 0) {
				currency = curr;
			} else {
				curr = CookieUtils.getCookie(COOKIE_CURRENCY, httpCtx);
