...
public class Photo extends Controller {
	...
	public Result photohome(int page,int pageSize,int status){
		Page<InteractionProductMemberPhotos> vlist = productReviewsService.getPhotoPage(page, pageSize,status);
		return ok(views.html.manager.review.photo_manager.render(vlist,status));
	}
	...
	public Result verify(){
		...
		if(iid!=null && !"".equals(iid)&& istatus!=null&& !"".equals(istatus)){
			...
			boolean flag = productReviewsService.verifyPhoto(Integer.parseInt(iid), st,
					verifyUser);
			if(flag && st==1 && points!=null && !"".equals(points) && commentid.length()==0){
				...
				pointService.grantPoints(email, sid,p ,
						"upload photo","upload photo",1,"upload photo");
			}
	...
	public Result batchverify(){
		...
		if(!StringUtils.isEmpty(batchstring) && !StringUtils.isEmpty(points) && 
			...
			for(String ss : s){
				...
				productReviewsService.verifyPhoto(iid, st, verifyUser);
				if(st==1 && commentid==0){
					pointService.grantPoints(email, sid,p ,
							"upload photo","upload photo",1,"upload photo");
				}
