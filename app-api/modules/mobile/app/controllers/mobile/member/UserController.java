...
public class UserController extends TokenController {
	...
	public Result peronInfo() {
		...
		try {
			...
			pMap.put("msg", MsgUtils.msg(BaseResultType.SUCCESSMSG));
		} catch (Exception e) {
			HashMap<String, Object> objMap = new HashMap<String, Object>();
			objMap.put("re", BaseResultType.EXCEPTION);
			objMap.put("msg", MsgUtils.msg(BaseResultType.EXCEPTIONMSG));
		...
		try {
			...
			if (b == false) {
				return ok(Json.toJson(new BaseJson(BaseResultType.FAILURE,
						MsgUtils.msg(BaseResultType.OPERATE_FAIL))));
			}
			result.setRe(BaseResultType.SUCCESS);
			result.setMsg(MsgUtils.msg(BaseResultType.SUCCESSMSG));
		} catch (Exception e) {
	...
	public Result points(int page, int pageSize) {
		...
		try {
			...
			pMap.put("msg", MsgUtils.msg(BaseResultType.EXCEPTIONMSG));
	...
	private BaseJson validateUserForm(UserInfoForm userform) {
		...
		if (ValidataUtils.validateNull(userform.getNickname()) == false
			...
			result.setMsg(MsgUtils.msg(BaseResultType.NICK_NAME_ERROR_MSG));
			return result;
		}
		if (ValidataUtils.validateNull(userform.getFname()) == false
			...
			result.setMsg(MsgUtils.msg(BaseResultType.FIRST_NAME_ERROR_MSG));
			return result;
		}
		...
		if (userform.getGender() == null) {
			...
			result.setMsg(MsgUtils.msg(BaseResultType.GENDER_IS_NULL_ERROR_MSG));
			return result;
		}
		if (userform.getGender() > 2 || userform.getGender() <= 0) {
			result.setRe(BaseResultType.GENDER_VALUE_ERROR_CODE);
			result.setMsg(MsgUtils.msg(BaseResultType.GENDER_VALUE_ERROR_MSG));
			return result;
		}
		...
		if (ValidataUtils.validateLength(userform.getCountry(), 3) == false) {
			...
			result.setMsg(MsgUtils
					.msg(BaseResultType.COUNTRY_LENGTH_OVER_ERROR_MSG));
			return result;
		}
		if (ValidataUtils.validateLength(userform.getAbout(), 3000) == false) {
			...
			result.setMsg(MsgUtils
					.msg(BaseResultType.ABOUT_LENGTH_OVER_ERROR_MSG));
			return result;
		}
		result.setRe(BaseResultType.SUCCESS);
		return result;
	...
	public Result uploadUserImg() {
		...
		try {
			...
			boolean b = userSvc.updatePhoto(email, siteId, contenttype,
					bos.toByteArray());
			if (b == false) {
				return ok(Json.toJson(new BaseJson(BaseResultType.FAILURE,
