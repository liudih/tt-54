...
public class BaseApi extends TokenController {
	@Inject
	ICountryService countryEnquiryService;
	@Inject
	ILanguageService languageService;
	@Inject
	IEmailSuffixService emailSuffixService;
	@Inject
	...
	public Result getCountry(int maxid) {
		...
		if (mid > maxid) {
			...
			cj.setMsg(MsgUtils.msg(BaseResultType.SUCCESSMSG));
	...
	public Result getLanguage(int maxid) {
		...
		if (mid > maxid) {
			...
			List<dto.mobile.Language> list2 = Lists.transform(clist, c -> {
				...
				d.setCname(c.getCfullname());
				d.setIid(c.getIid());
				d.setCurrency(MateCurrencyUtils.getCurrencyCode(c.getIid()));
				return d;
			});
