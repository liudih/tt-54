...
public class MobileService {
	...
	public boolean setMLoginContext(MobileLoginContext context) {
		if (context != null) {
			return sessionServer.setMLoginContext(context);
		}
		return false;
	}

	public MobileLoginContext getMLoginContext() {
		return sessionServer.getMLoginContext();
	}
	...
	public int getLanguageID() {
		...
		return StringUtils.isBlank(strlanguageid) ? 1
	...
	public int getWebSiteID() {
		...
		return StringUtils.isBlank(strsiteid) ? 1 : (NumberUtils
				.isNumeric(strsiteid) ? Integer.parseInt(strsiteid) : 1);
	}
	...
	public String getCurrency() {
		...
		return StringUtils.isNotBlank(currency) ? currency : "USD";
	}
	...
	public WebContext getWebContext() {
		...
		if (ctx != null) {
			...
			cookies.add(new WebCookie("TT_DEVICE", "app",
					30 * 24 * 3600 * 1000, "", ctx.getHost()));
			MobileLoginContext mctx = getMLoginContext();
			if (mctx != null) {
				cookies.add(new WebCookie("TT_TOKEN", mctx.getCookieToken(),
						7 * 24 * 3600 * 1000, "", ctx.getHost()));
				cookies.add(new WebCookie("TT_UUID", mctx.getCookieUUID(),
						7 * 24 * 3600 * 1000, "", ctx.getHost()));
			}
