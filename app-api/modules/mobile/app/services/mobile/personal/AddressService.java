...
public class AddressService {
	...
	public boolean setDefaultAddress(Integer id, String email,
		...
		if (StringUtils.isNotBlank(email) && id != null) {
			MemberAddress adress = addressService.getMemberAddressById(id);
			if (adress != null
					&& email.equalsIgnoreCase(adress.getCmemberemail())) {
				try {
					addressService.setDefaultShippingaddress(id, email,
							addressType);
					return true;
				} catch (Exception e) {
	...
	public boolean updateMemberAddress(MemberAddress memberAddress) {
		...
		boolean result = addressService
				.updateMemberShippingAddress(memberAddress);
		return result;
	}
	...
	public List<AddressBaseInfo> getMemberShippingAddressByEmail(String email,
		...
		if (type == 1) {
			memberAddress = addressService
					.getMemberShippingAddressByEmail(email);
		} else if (type == 2) {
			memberAddress = addressService.getOrderAddressByEmail(email);
		} else {
			return null;
		}
	...
	public boolean addNewAddress(MemberAddress memberAddress) {
		...
		boolean reuslt = addressService.addNewAddress(memberAddress);
		return reuslt;
	}
	...
	public List<MemberAddress> getOrderAddressByEmail(String email) {
		return addressService.getOrderAddressByEmail(email);
	}

	public MemberAddress getDefaultOrderAddress(String email) {
		return addressService.getDefaultOrderAddress(email);
	}
	...
	public AddressInfo getAddressInfoById(Integer id) {
		MemberAddress adress = addressService.getMemberAddressById(id);
		return this.getAddressInfo(adress);
	}

	public Integer getShippingAddressCountByEmail(String email) {
		return addressService.getShippingAddressCountByEmail(email);
	}

	public Map<String, Object> getMemberAddressMapById(Integer id) {
		MemberAddress memberAddress = addressService.getMemberAddressById(id);
	...
	public AddressBaseInfo getDefaultAddress(String email, Integer type) {
		...
		if (type == 1) {
			memberAddress = addressService.getDefaultShippingAddress(email);
		} else if (type == 2) {
			memberAddress = addressService.getDefaultOrderAddress(email);
		} else {
			return null;
		}
	...
	private AddressBaseInfo getAddressBaseInfo(MemberAddress memberAddress) {
		...
		Country country = countryService.getCountryByCountryId(memberAddress
				.getIcountry());
		String countryName = "";
		if (country != null) {
