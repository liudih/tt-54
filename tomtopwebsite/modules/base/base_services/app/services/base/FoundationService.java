...
public class FoundationService implements IFoundationService {
	...
	public String getLang() {
		...
		Cookie lang = ctx.request().cookie("PLAY_LANG");
		if (lang != null) {
			String v = lang.value();
			return v;
		}
	...
	private int getLanguageFromQuery(Context ctx) {
		...
		if (null != langstr) {
			...
			if (langObj != null) {
				...
				Logger.debug("---->get querystring lang --{}", langstr);
	...
	public int getLanguage(WebContext ctx) {
		String langstr = (String) ctx.get("lang");
		if (null != langstr) {
			langstr = langstr.toLowerCase();
			Language langObj = this.langService.getLanguageByCode(langstr);
			if (langObj != null) {
				this.setLanguage(langObj.getIid());
				Logger.debug("---->WebContext querystring lang --{}", langstr);
				return langObj.getIid();
			}
		}
	...
	public void setLanguage(Context ctx, int langId) {
		...
		if (lang != null) {
			// CookieUtils.removeCookie(Play.langCookieName(), ctx);
	...
	public String getCurrency(Context ctx) {
		...
		// ~ 添加url货币参数 fcl
		String currency = getCurrencyFromQueryString(ctx);
	...
	private String getCurrencyFromQueryString(Context ctx) {
		// ~ 添加url货币参数 fcl
	...
	public String getCurrency(WebContext ctx) {
		...
		if (currency != null) {
			...
			if (null != currencyService.getCurrencyByCode(currency)) {
				this.setCurrency(currency);
				Logger.debug("webContext query curr--->{} ", currency);
				return currency;
			}
		}
	...
	public WebContext getWebContext() {
		...
		String currency = Context.current().request()
				.getQueryString("currency");
		if (currency != null) {
			wc.set("currency", currency);
		}
		String langstr = Context.current().request().getQueryString("lang");
		if (null != langstr) {
			wc.set("lang", langstr);
		}
		return wc;
