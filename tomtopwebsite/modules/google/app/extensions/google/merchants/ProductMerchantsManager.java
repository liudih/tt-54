...
import com.google.api.services.content.ShoppingContent.Inventory.Custombatch;
...
public class ProductMerchantsManager extends BaseMerchantsProduct {
		...
		if (baseProductList.size() == 1) {
			...
			if (product != null) {
				...
				try {
					...
					Logger.debug("insertProduct----------------------return Product:"+result);
					if(result!=null){
						...
						try{
							...
							Logger.error("insertProduct bean2xml error:", e3);
						}
					}
				} catch (IOException e) {
	...
	private void merchantsProductBatch(
		...
		try {
			...
			int count=0;
			Logger.debug("merchantsProductDtoMap.keySet().size:"+merchantsProductDtoMap.keySet().size());
			for(String nodeId :merchantsProductDtoMap.keySet()){
			...
			if(CollectionUtils.isNotEmpty(productsBatchRequestEntries)){
				...
				if (batchResponse != null && CollectionUtils.isNotEmpty(batchResponse.getEntries())) {
					Logger.debug("merchantsProductBatch batchResponse.getEntries().size:"+batchResponse.getEntries().size());
					for (ProductsCustomBatchResponseEntry entry : batchResponse
						...
						String jsonProduct=JSON.toJSONString(product);
						Logger.debug("merchantsProductBatch create product:"+jsonProduct);
						Map<String, Object> faultReason = Maps.newHashMap();
						if (product!=null) {
							...
							Logger.debug("merchantsProductBatch ----------->nodeId:"+nodeId);
							if(merchantsProductDtoMap.containsKey(nodeId) && merchantsProductDtoMap.get(nodeId)!=null){
									...
									if( MapUtils.isNotEmpty(faultReason)){
										merchantsProductDto.setCfaultreason(faultReason.toString());
										merchantsProductDto.setCresult(FAIL);
	...
	public ProductBase getProduct(String id) {
		...
		try {
			...
			if (product != null) {
				...
				productbase.setCsku(product.getMpn());
	...
	public GoogleMerchantsProductDto getGoogleProductMap(String nodeData){
		...
		Map<String,Class> map=Maps.newHashMap();
		Logger.debug("getGoogleProductMap product change start!");
		Product product=(Product)XMLBeanUtils.xml2Bean(map,nodeData);
		Logger.debug("getGoogleProductMap change end product:"+product);
		if(product!=null){
	...
	private GoogleMerchantsProductDto conver2GoogleProduct(Product product){
		...
		googleMerchantsProductDto.setOfferId(product.getMpn());
		...
		Logger.debug("conver2GoogleProduct  googleMerchantsProductDto:"+googleMerchantsProductDto);
	...
	public void updateProductPriceAndAvailability(List<MerchantsProductDto> list){
		if(CollectionUtils.isEmpty(list)){
			Logger.info("updateProductPriceAndAvailability list empty");
		}
		...
		try{
			...
			if(MapUtils.isEmpty(feedMap)){
				feedMap = getFeedLangeCountryMap();
				Logger.debug("updateProductPriceAndAvailability feedMap:"+feedMap);
			}
			InventoryCustomBatchRequest inventoryContext=new InventoryCustomBatchRequest();
			List<InventoryCustomBatchRequestEntry> entries=Lists.newArrayList();
			List<String> nodeIdList=Lists.newArrayList();
			setCredential();
			for(int i=0;i<list.size();i++ ){
				Long index=new Long(i);
				MerchantsProductDto merchantsProductDto=list.get(i);
				Logger.info("updateProductPriceAndAvailability cnodeId:"+merchantsProductDto.getCnodeid());
				InventoryCustomBatchRequestEntry requestEntry=new InventoryCustomBatchRequestEntry();
				
				requestEntry.setBatchId(index);
				requestEntry.setMerchantId(merchantId);
				requestEntry.setProductId(merchantsProductDto.getCnodeid());
				requestEntry.setStoreCode("online");
				
				Inventory inventory=new Inventory();
				try{
					updatePriceInventory(merchantsProductDto,feedMap);
				}catch(Exception e){
					Logger.error("updateProductPriceAndAvailability sku:"+merchantsProductDto.getCsku()+
							"  update price fail",e);
					continue;
				}
				Price salePrice =new Price();
				salePrice.setCurrency(merchantsProductDto.getCcountrycurrency());
				salePrice.setValue(merchantsProductDto.getProductprice());
				inventory.setPrice(salePrice);
				inventory.setKind("content#inventory");
				String availability=merchantsProductDto.getIcount()>0?"in stock":"out of stock";
				inventory.setAvailability(availability);//out of stock  
				requestEntry.setInventory(inventory);
				map.put(merchantsProductDto.getCnodeid(),index);
				entries.add(requestEntry);
				nodeIdList.add(merchantsProductDto.getCnodeid());
			}
			
			Logger.debug("updateProductPriceAndAvailability  nodeIdList:"+nodeIdList);
			//requestEntry.set
			inventoryContext.setEntries(entries);
			extensions.google.googlemerchantback.ShoppingContentBack.Inventory.Custombatch custombatch
														= contentBack.inventory().custombatch(inventoryContext);
			InventoryCustomBatchResponse batchResponse = custombatch.execute();
			
			Logger.debug("updateProductPriceAndAvailability batchResponse:"+batchResponse);
			if(batchResponse!=null ){
				if(batchResponse.getEntries()!=null){
					setInventoryRecordState(list,batchResponse.getEntries(),map,null);
				}else{
					setInventoryRecordState(list,batchResponse.getEntries(),map,"batchResponse getEntries empty");
					Logger.info("batchResponse getEntries enpty batchResponse:"+batchResponse);
				}
			}else{
				setInventoryRecordState(list,null,map,"batchResponse null");
				Logger.info("batchResponse null ");
			}
		}catch(Exception e){
			setInventoryRecordState(list,null,map,"updateProductPriceAndAvailability error");
			Logger.error("updateProductPriceAndAvailability error!",e);	
		}
	}
	private void updatePriceInventory(MerchantsProductDto merchantsProductDto,Map<String,Integer> feedMap) throws Exception{
		if(MapUtils.isEmpty(feedMap)){
			merchantsProductDto.setCfaultreason("updatePriceInventory update product price fail! feed map empty");
			throw new ServiceException("updatePriceInventory update product price fail! feed map empty");
		}
		String key=merchantsProductDto.getClanguage()+"_"+merchantsProductDto.getCtargetcountry();
		if(feedMap.containsKey(key)){
			//Integer integer = feedMap.get(key);
			ProductBase productBaseByParams = productBaseMapper.getProductBaseBySkuAndSite(merchantsProductDto.getCsku(), 1);
			if(productBaseByParams==null){
				productBaseByParams = productBaseMapper.getProductBaseBySkuAndSite(merchantsProductDto.getCsku(), 1);
			}
			if(productBaseByParams==null || MapUtils.isEmpty(feedMap)){
				merchantsProductDto.setCfaultreason("update product price fail! ProductBase null  cnodeId:"+merchantsProductDto.getCnodeid()+"  feedMap:"+feedMap);
				throw new ServiceException("updatePriceInventory productBaseByParams listingId NULL or feedMap empty");
			}
			String listingId=productBaseByParams.getClistingid();
			
			Logger.info("updatePriceInventory merchantsProductDto cnodeId:"+merchantsProductDto.getCnodeid());
			
			merchantsProductDto.setIcount(productBaseByParams.getIqty());
			...
			if (isCurrencyExist>0 && StringUtils.isNotEmpty(merchantsProductDto.getCcountrycurrency())) {
				...
				// 创建Get方法实例
				String productUrl = Play.application().configuration()
						.getString("productservice.url");
				if (StringUtils.isEmpty(productUrl)) {
					Logger.error("product website url is empty!!");
				}
				
				String url = productUrl + "/ic/v2/products/"+listingId+"?currency=" + merchantsProductDto.getCcountrycurrency()
						+ "&website=1";
...
//						listingId, merchantsProductDto.getCcountrycurrency());
				if (data != null) {
					...
					Double p = jsonNode.get("data").get("nowprice")
							.asDouble();
					merchantsProductDto.setProductprice(p.toString());
					Logger.debug("---------------->:" + listingId+"  price:"+p);
					
				}else{
					Logger.debug("updatePriceInventory price fail :listingid:"+listingId+"   currency:"+merchantsProductDto.getCcountrycurrency());
					merchantsProductDto.setCfaultreason("update product price fail!listingid:"+listingId+"   currency:"+merchantsProductDto.getCcountrycurrency());
					throw new ServiceException("update product price fail");
				}
			}
		}else{
			throw new ServiceException("updatePriceInventory update product price fail! feed map not containsKey:"+key+"  feedMap:"+feedMap);
		}
	}
	private void setInventoryRecordState(List<MerchantsProductDto> list,
			List<InventoryCustomBatchResponseEntry> Entrys,Map<String,Long> map,String failReason){
		if(CollectionUtils.isEmpty(list) ){
			Logger.info("setInventoryRecordState list empty ");
			return;
		}
		if(StringUtils.isEmpty(failReason)){
			for(MerchantsProductDto merchantsProductDto:list){
				merchantsProductDto.setCstate("2");
				merchantsProductDto.setCresult("fail");
				merchantsProductDto.setCfaultreason(failReason);
				merchantsProductDto.setDupdatedate(new Date());
			}
			return;
		}
		if(MapUtils.isEmpty(map)){
			Logger.info("setInventoryRecordState map empty ");
			return;
		}
		Logger.debug("setInventoryRecordState map:"+map);
		if(CollectionUtils.isEmpty(Entrys)){
			for(MerchantsProductDto item : list){
				item.setCstate("2");
				item.setCresult("fail");
				item.setDupdatedate(new Date());
			}
		}else{
			Map<Long,InventoryCustomBatchResponseEntry> responseMap=Maps.newHashMap();
			for (InventoryCustomBatchResponseEntry entry : Entrys ) {
				Logger.debug("setInventoryRecordState entry:"+entry);
				responseMap.put(entry.getBatchId(), entry);
			}
			for (MerchantsProductDto merchantsProductDto : list ) {
				String cnodeid = merchantsProductDto.getCnodeid();
				merchantsProductDto.setDupdatedate(new Date());
				if(StringUtils.isNotEmpty(cnodeid) ){
					if(map.containsKey(cnodeid)){
						Long long1 = map.get(cnodeid);
						if(responseMap.containsKey(long1)){
							InventoryCustomBatchResponseEntry inventoryCustomBatchResponseEntry = responseMap.get(long1);
							Logger.debug("setInventoryRecordState ------->inventoryCustomBatchResponseEntry:"+inventoryCustomBatchResponseEntry);
							Errors errors = inventoryCustomBatchResponseEntry.getErrors();
							if(errors!=null){
								merchantsProductDto.setCstate("2");
								merchantsProductDto.setCresult("fail");
								String jsonString = JSON.toJSONString(errors);
								merchantsProductDto.setCfaultreason(jsonString);
							}else{
								merchantsProductDto.setCstate("2");
								merchantsProductDto.setCresult("success");
							...
							merchantsProductDto.setCstate("2");
							merchantsProductDto.setCresult("fail");
							Logger.info("setInventoryRecordState responseMap not contain long1:"+long1+"cnodeId :"+cnodeid);
						}
					}else{
						merchantsProductDto.setCstate("2");
						merchantsProductDto.setCresult("fail");
						Logger.info("setInventoryRecordState map not contain cnodeId :"+cnodeid);
					}
				}else{
					Logger.info("setInventoryRecordState merchantsProductDto cnodeId empty! sku:"+merchantsProductDto.getCsku());
				}	
			}
