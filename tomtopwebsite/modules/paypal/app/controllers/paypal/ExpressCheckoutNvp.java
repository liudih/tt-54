...
import valueobjects.order.OrderConfirmForm;
...
/*			boolean isSameStorage = shippingServices.isSameStorage(listingId,
			...
			if (loginCtx != null && loginCtx.isLogin()) {
				...
				prefers = prefer.getAllPreferByEmail("", items, webCtx);
			}
	...
	public Result confirmOrder() {
		...
		List<valueobjects.cart.CartItem> newItems = orderService.transToNewCartItems(items);
		// 开始核对邮费方式是否正确
		valueobjects.order.ShippingMethod hit = orderService.checkShippingMethodCorrect(
				originalOrder.getIstorageid(), countryCode, shipCode,
				originalOrder.getFordersubtotal(), newItems, currency, lang);
		if (hit == null) {
			return redirect(controllers.order.routes.OrderProcessing
