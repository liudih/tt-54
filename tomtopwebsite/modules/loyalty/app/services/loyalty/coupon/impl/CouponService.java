...
public class CouponService implements ICouponMainService,
	...
	public Page<Coupon> selectForPage(int page, int pageSize, String email,
		...
		List<CouponValue> couponValuelist = this.dao.selectForPage(paras);
		List<Coupon> list = new ArrayList<Coupon>();
		for(CouponValue cv : couponValuelist){
			Coupon c = new Coupon();
			c.setCode(cv.getCode());
			c.setCodeId(cv.getCodeId());
			c.setCreateDate(cv.getCreateDate());
			c.setCreator(cv.getCreator());
			c.setCreatorName(cv.getCreatorName());
			c.setCurrency(cv.getCurrency());
			c.setDiscount(cv.getDiscount());
			c.setEmail(cv.getEmail());
			c.setId(cv.getId());
			c.setMinAmount(cv.getMinAmount());
			c.setModifierId(cv.getModifierId());
			c.setModifyDate(cv.getModifyDate());
			c.setOrderId(cv.getCodeId());
			c.setOrderNumber(cv.getOrderNumber());
			c.setPar(cv.getPar());
			c.setParentId(cv.getParentId());
			c.setRuleId(cv.getRuleId());
			c.setRuleName(cv.getRuleName());
			c.setStatus(cv.getStatus());
			c.setTimes(cv.getTimes());
			c.setTimeType(cv.getTimeType());
			c.setType(cv.getType());
			c.setUsedtimes(cv.getUsedtimes());
			c.setValidDays(cv.getValidDays());
			c.setValidEndDate(cv.getValidStartDate());
			c.setValidStartDate(cv.getValidStartDate());
			c.setValue(cv.getValue());
			c.setValueType(cv.getValueType());
			if(cv.getWebsiteId() != null){
				if(cv.getWebsiteId().indexOf(",") != -1){
					String[] strs = cv.getWebsiteId().split(",");
					c.setWebsiteId(Integer.parseInt(strs[0]));
				}else{
					c.setWebsiteId(Integer.parseInt(cv.getWebsiteId()));
				}
			}
			list.add(c);
		}
	...
	public List<Coupon> giftCouponForRSS(String userEmail, int siteId) {
		...
		if (existed) {
			Logger.info("{}---Has been giving coupons", userEmail);
			return codes;
		}
		// 获取发送优惠券张数配置参数
		...
		if (!StringUtils.isEmpty(numsStr)) {
			try {
				...
				if (Logger.isInfoEnabled()) {
					Logger.info(
							"<<<<<<<<<<<<<<<<<<<<<<numsCouponForRss in table t_system_parameter is invalid{}",
							userEmail);
				}
			}
		...
		for (int i = 0; i < nums; i++) {
			...
			if (0 != codeId) {
				...
				Logger.error(
						"<<<<<<<<<<<Generate coupons ID failure<<<<<<<<<<<get {}th RSS coupon code is failed for {}",
						i, userEmail);
			}
		}
		return codes;
					...
					// 设置开始结束日期
						if (c.getParameter().getTimeType() != null) {
							...
							case VALIDITY:
								...
								Calendar calendar = Calendar.getInstance();
								calendar.setTime(start);
								Integer validity = c.getParameter()
										.getIvalidity();
								calendar.add(calendar.DATE, validity);
								// 结束时间
								Date end = calendar.getTime();
								coupon.setStartDate((start.getYear() + 1900)
										+ "." + (start.getMonth() + 1) + "."
										+ start.getDate());
								coupon.setEndDate(calendar.get(Calendar.YEAR)
										+ "."
							...
							case DATE:
								// 开始时间
								Date startDate = c.getParameter()
										.getStartdate();
								Date endDate = c.getParameter().getEnddate();
								coupon.setStartDate((startDate.getYear() + 1900)
										+ "."
										+ (startDate.getMonth() + 1)
										+ "." + startDate.getDate());
								coupon.setEndDate((endDate.getYear() + 1900)
										+ "." + (endDate.getMonth() + 1) + "."
										+ endDate.getDate());
								break;
							}
						...
						if (CouponRuleBack.CouponType.CASH == c.getParameter()
							...
							Integer currencyId = c.getParameter()
									.getCcurrency();
							if (null != currencyId) {
	...
	public Coupon createCouponByRuleNoContext(String email, Integer ruleId,
			int siteid) {
		if (StringUtils.isEmpty(email) || null == ruleId) {
			throw new NullPointerException("email or ruleId is empty,email=="
					+ email + "ruleId==" + ruleId);
