...
public class PromoCodeService implements IPromoCodeService,
		...
		if (couponLine != null) {
			...
			if (codeNode != null) {
				...
				ConponActionRule actionRule = couponService
						.getActionRuleForPormoCode(code);
				if (actionRule != null) {
					CampaignContext context = ctxFactory.createContext(null,
							null);
					// 把pc端的上下文设置进去
	...
	@Override
	public boolean editPromoAssociateRule(Integer promoId, Integer ruleId) {
		int result = this.dao.editPromoAssociateRule(promoId, ruleId);
