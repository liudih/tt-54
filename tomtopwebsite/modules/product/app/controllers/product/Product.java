...
import valueobjects.product.ProductBadgeConvert;
...
public class Product extends Controller {
	...
	public Result view(final String title) {
		...
		Logger.debug("product---->   {}--{}-{}", foundation.getLanguage(),
				foundation.getCurrency(), foundation.getClientIP());
	...
	public Result newarrivals(int type, Integer page, Integer size, Integer st,
		...
		Logger.info(" -------newarrivals in ----------------------");
		Logger.info("-------foundation----"+JSON.toJSONString(foundation));
		...
		List<ProductNewarrivalsCalculateItem> newarrivalsList = productEnquiryService
				.findAllNewarrivalsGroupByCreateDate(statDate,
						Integer.parseInt(newValidDays), foundation.getSiteID());
		...
		int site = foundation.getSiteID();
		String currency = foundation.getCurrency();
		Logger.info("newarrivals--currency---="+currency+",lang="+lang+",site="+site);
		...
		if(badges!=null && badges.size()>0){
			Logger.info("--Newarrivals--badges="+JSON.toJSONString(badges.get(0)));
		}
		// 产品分页数据
	...
	private void getNewarrivalsByDate(
			...
			Integer newValidDays, Integer iwebsiteid) {
	 ...
	 */
	public Result dailyDeal(int page, int type) throws ClientProtocolException,
			IOException {
...
//		});
		...
		// d. 不包含产品：特殊产品(清仓品)
		// String url =
		List<ProductBadgeConvert> productBadgeConverts = getProductBadges(
				currency, lang, type);
		Logger.info("dailyDeal-start==1===");
		...
		Logger.info("dailyDeal-start==2===");
		Page<String> listingids = genericSearch.search(context, site, lang);
		
		Logger.info("dailyDeal-start==3==={}",listingids.getList());
		Logger.info("dailyDeal-start==3==={}",listingids.getList().size());

		Page<ProductBadge> badgePage = listingids.batchMap(list -> badgeService
				.getProductBadgesByListingIDs(list, lang, site, currency, date,
						false, true));
		Logger.info("dailyDeal-start==4===");
		// 获取有产品的类目
		List<CategoryWebsiteWithName> catelist = categoryEnquiryService
		...
		return ok(views.html.product.dailydeal.render(productBadgeConverts,
				badgePage, catelist, type, intdiff, context, request()
						.queryString(), subscribeHtmls));
	}
	...
	public Result hot(int p, int limit) {
		...
		Page<String> listingids = genericSearch.search(context, websiteid,
				languageid);
		String currency = foundation.getCurrency();
		Logger.info("--hot---languageid="+languageid+",websiteid="+websiteid+",currency="+currency);
		Page<ProductBadge> badgePage = listingids.batchMap(list -> badgeService
				.getProductBadgesByListingIDs(list, languageid, websiteid,
						currency, null));
		if(badgePage!=null && badgePage.getList()!=null &&badgePage.getList().size()>0){
			Logger.info("--hot---badgePage="+JSON.toJSONString(badgePage.getList().get(0)));
		}
		// 获取广告
	...
	public Result freeShipping() {
		...
		int site = foundation.getSiteID();
		String currency = foundation.getCurrency();
		Logger.info("---freeShipping--lang="+lang+",site="+site+",currency="+currency);
		...
		SearchContext context = searchFactory.fromQueryString(pt, request()
				.queryString(), Sets
				.newHashSet(enumtype.ProductLabelType.FreeShipping.toString()));
		// Page<String> listingids = genericSearch.search(context, site, lang);
		//
		...
		Page<String> listingIDs = genericSearch.search(context, site, lang);
		Logger.debug("-->freeshipping query-->{}--{}",
				Json.toJson(listingIDs.getList()),
				enumtype.ProductLabelType.FreeShipping.toString());
		List<ProductBadge> badges = badgeService
				.getNewProductBadgesByListingIds(listingIDs.getList(), lang,
						site, currency, null);
		if(badges!=null && badges.size()>0){
			Logger.info("--freeShipping--badges="+JSON.toJSONString(badges.get(0)));
		}
		// 产品分页数据
		Map<String, ProductBadge> badgeMap = Maps.newHashMap();
		for (ProductBadge b : badges) {
			Logger.debug("---{}", b.getListingId());
			badgeMap.put(b.getListingId(), b);
		}
		Page<ProductBadge> badgePage = listingIDs.map(id -> badgeMap.get(id));
	...
	private List<ProductBadgeConvert> getProductBadges(String currency,
			int lang, int type) throws ClientProtocolException, IOException {
		...
		String url = productUrl + "/ic/v1/home/dailyDeal?currency=" + currency
				+ "&client=1&lang=" + lang + "&date="
				+ UTCTimeBuffer.toString();
		Logger.info("productUrl======{}",url);
		HttpGet httpgets = new HttpGet(url);
		HttpResponse response = httpclient.execute(httpgets);
		HttpEntity entity = response.getEntity();
		List<ProductBadgeConvert> productBadgeConverts = null;
		if (entity != null) {
			InputStream instreams = entity.getContent();
			String str = convertStreamToString(instreams);
			productBadgeConverts = convertProductBadges(str);
			Logger.info("product===str======{}",str);
			httpgets.abort();
		}
		return productBadgeConverts;
	}

	private List<ProductBadgeConvert> convertProductBadges(String jsonStr) {
		JSONObject jo = (JSONObject) JSONObject.parse(jsonStr);
		String key = "data";
		JSONArray value = jo.getJSONArray(key);
		List<ProductBadgeConvert> products = Lists.newArrayList();
		if (null != value) {
			products = JSON.parseArray(value.toJSONString(),
					ProductBadgeConvert.class);
			if (null != products && products.size() > 0) {
				products.forEach(e -> {
					e.setCurrencySymbol(currencyService.getCurrencyByCode(
							foundation.getCurrency()).getCsymbol());
				});
			}
		}
		return products;
	...
	public String convertStreamToString(InputStream is) {
		...
		try {
			...
			Logger.error("transform error!-----class=Product method=convertStreamToString");
		} finally {
			try {
				is.close();
			} catch (IOException e) {
