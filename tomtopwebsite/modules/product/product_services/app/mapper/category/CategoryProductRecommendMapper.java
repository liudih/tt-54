...
public interface CategoryProductRecommendMapper {
	...
	@Select("select categoryid,sku,level,sequence,iwebsiteid from t_category_product_recommend where status = 1")
	List<CategoryProductRecommend> getCategoryRecommends();

	@Select("select sku,level from t_category_product_recommend where iid = #{0}")
	CategoryProductRecommend getcprByIid(Integer id);

	@Select("select categoryid,sku,level,sequence,iwebsiteid from t_category_product_recommend where status = 1 and categoryid=#{0}")
	List<CategoryProductRecommend> getCategoryRecommendsByCid(Integer categoryId);
}
