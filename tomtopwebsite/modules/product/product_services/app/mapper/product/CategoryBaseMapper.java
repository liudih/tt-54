...
public interface CategoryBaseMapper {
	...
	@Select("select iid from t_category_website where cpath=#{0} limit 1 ")
	Integer getCategorieIdByPath(String cpath);

	@Select("select iid from t_category_base where cpath=#{0}")
	Integer getCategoryBaseIdByCpath(String cpath);
