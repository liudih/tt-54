...
public class ProductEnquiryService implements IProductEnquiryService {
	...
	public List<ProductLite> getProductLiteByListingIDs(
		...
		if (listingIDs != null && listingIDs.size() > 0) {
			...
			try {
				Logger.debug("getProductLiteByListingIDs--{}--{}--{}",
						Json.toJson(listingIDs), websiteID, languageID);
				plist = productBaseMapper.getProductByListingIDs(listingIDs,
						websiteID, languageID);
				Logger.debug("getProductLiteByListingIDs--result--{}",
						Json.toJson(plist));
			} catch (Exception ex) {
				Logger.error("getProductLiteByListingIDs--error", ex);
			}
			return plist;
		}
	...
	public List<ProductNewarrivalsCalculateItem> findAllNewarrivalsGroupByCreateDate(
			Date statDate, Integer newValidDays, Integer iwebsiteid) {
