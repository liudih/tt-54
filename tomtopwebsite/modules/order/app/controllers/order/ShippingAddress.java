...
import services.order.ShippingApiServices;
...
public class ShippingAddress extends Controller {
	 ...
	 * @return
	 */
	public Result getNewShipMethod(String country, Double totalPrice,
			Integer storageid, String orderNumber) {
		Map<String,Object> mjson = new HashMap<String,Object>();
		...
		if(orderNumber!=null && !"".equals(orderNumber)){
			List<valueobjects.order_api.cart.CartItem> oitems = orderDetailDao
					.selectCartItemsByOrderNum(orderNumber);
			items = orderService.transToNewCartItems(oitems);
			if(items.size()==0){
				mjson.put("succeed", false);
