...
public interface CategoryWebsiteMapper {
	...
	@Select("SELECT p.*, n.cname FROM t_category_website p "
			...
			+ "WHERE  p.icategoryid = #{0}  and n.ilanguageid =#{1} AND p.iwebsiteid = #{2} limit 1 ")
	CategoryWebsiteWithName getCategoryWebsiteByIdAndSiteIdAndLangId(int id, int languageid,
			int websiteid);
	...
	@Select("SELECT b.* ,n.cname from t_category_base b INNER JOIN t_category_name n on"
			+ " b.iid = n.icategoryid where n.ilanguageid = 1 and b.iparentid is NULL")
	List<CategoryWebsiteWithName> getFirstCategory();

	@Select("SELECT b.* ,n.cname from t_category_base b INNER JOIN t_category_name n on"
			+ " b.iid = n.icategoryid where n.ilanguageid = 1 and b.iparentid = #{0}")
