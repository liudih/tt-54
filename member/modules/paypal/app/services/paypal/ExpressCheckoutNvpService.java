...
public class ExpressCheckoutNvpService implements IExpressCheckoutNvpService {
	...
	public PaypalNvpPaymentStatus DoExpressCheckoutPayment(String token,
		...
		// Map<String, String> detail = GetExpressCheckoutDetails(token,
		// PayerID);
		// String orderNum = detail.get("PAYMENTREQUEST_0_INVNUM");
		// if (orderNum == null || orderNum.length() == 0) {
		// status.setFailedInfo("can not get orderNum from GetExpressCheckoutDetails response");
		// return status;
		// }
				...
				.map(resp -> {
					...
					if (response.containsKey("PAYMENTINFO_0_ACK")) {
						...
						if ("success".equals(ack.toLowerCase())) {
							...
							if (!dropShip) {
								PaymentEvent event = new PaymentEvent(orderNum,
										state, transactionId, ttAccount, amt,
										"paypal");
								eventBus.post(event);
							}
