...
public class MerchantsService implements IMerchantsService{
	...
	public String pushProductMerchantsService(List<MerchantsProductDto>  merchantsProductDtoList){
		...
		try{
			...
			for(MerchantsProductDto  merchantsProductDto :merchantsProductDtoList){
				//设置merchantsProductDto ID "id": "online:en:CA:L0160W",
				String nodeID=PRODUCT_CHANNEL+":"+merchantsProductDto.getClanguage()+":"+merchantsProductDto.getCtargetcountry()+":"+merchantsProductDto.getCsku();
		...
		try{
			...
			if(StringUtils.isNotEmpty(productId)){
				...
				if(CollectionUtils.isNotEmpty(validatePullResult)){
					result="fail records:"+om.convertValue(validatePullResult, String.class);
				}
				...
				if(CollectionUtils.isNotEmpty(failReacord)){
					result="fail records:"+om.convertValue(failReacord, String.class);
				}
			}
		}catch(Exception e){
	...
	public String pushMerchantsProductByLangeAndCountry(String lange,String country){
		...
		if(CollectionUtils.isNotEmpty(failList)){
			result="fail records:"+om.convertValue(failList, String.class);
		}
		return result;
	}
