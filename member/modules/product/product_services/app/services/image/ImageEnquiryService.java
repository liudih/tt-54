...
public class ImageEnquiryService implements IImageEnquiryService {
	...
	public Img getImageByPath(String path, boolean withContent) {
		if (withContent) {
			return imageMapper.getImageByPath(path);
		} else {
			return imageMapper.getImageByPathWithoutContent(path);
		}
	}

	public List<Img> getAllImageByPage(ImgPageForm pageForm) {
		return imageMapper.getImgByPage(pageForm.getContentType(),
				pageForm.getPath(), pageForm.getPageSize(),
				pageForm.getPageNum());
	}

	public Integer getImgCount(ImgPageForm pageForm) {
		return imageMapper.getImgCount(pageForm.getContentType(),
				pageForm.getPath());
	}
