...
/*	   启用多仓库就不用这个检查了
       boolean isSameStorage = shippingServices.isSameStorage(listingId,
		...
		Form<PlaceOrderV2> orderForm = Form.form(PlaceOrderV2.class)
				.bindFromRequest();
		...
		PlaceOrderV2 form = orderForm.get();
		Integer shippingMethodId = form.getShippingMethodIdValue();
		...
		CreateOrderRequest request = new CreateOrderRequest(items, site,
				addressId, shippingMethodId, origin, message, ip, lang,
