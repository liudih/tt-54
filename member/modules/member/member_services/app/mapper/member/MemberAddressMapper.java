...
public interface MemberAddressMapper {
	...
	@Select("select * from t_member_address where cmemberemail = #{0} and  iaddressid = 1 and (iwebsiteid=1 or iwebsiteid is null)"
			+ "offset (#{1}-1)*#{2} limit #{2}")
	List<MemberAddress> getShippingAddressByPage(String email,
			Integer pageIndex, Integer recordPerPage);

	@Select("select * from t_member_address where cmemberemail = #{0} and iaddressid = 2 and (iwebsiteid=1 or iwebsiteid is null)"
			+ "offset (#{1}-1)*#{2} limit #{2}")
	List<MemberAddress> getBillingAddressByPage(String email,
			Integer pageIndex, Integer recordPerPage);

	@Select("select count(iid) from t_member_address where cmemberemail = #{0} and iaddressid = 2 and (iwebsiteid=1 or iwebsiteid is null)")
	...
	@Select("select count(iid) from t_member_address where cmemberemail = #{0} and  iaddressid = 1 and (iwebsiteid=1 or iwebsiteid is null)")
