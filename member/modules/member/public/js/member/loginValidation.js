	 ...
	 $("#loginForm").validate({
		...
		messages: {
		  ...
		  highlight: function (error,element) { 
			  $(error).addClass("errorInp");
				$(element).addClass('edit_error');
		     },

		     unhighlight: function (element) { 
		    	$(element).removeClass('errorInp');
		    	$(element).removeClass('edit_error');
		    	$('#submit').removeAttr('disabled').addClass("log_sinIn");
		     },
	...
	this.each(function(){
		...
		if(type == "input" && inputType == "text" || inputType == "password"){
			...
			this.onkeypress = function(e){
				var ob = e || event;
				var keyCode = ob.keyCode;
				var hasSelection = document.selection? document.selction.createRange().text.length > 0 : this.selectionStart != this.selectionEnd;
				return !(this.value.length >= max && (keyCode > 50 || keyCode == 32 || keyCode == 0 || keyCode == 13) && !ob.altKey && !hasSelection );
			};
			this.onkeyup = function(){
				if(this.value.length > max){
					this.value = this.value.substring(0,max);
