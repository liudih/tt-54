...
import dto.mobile.OrderClassCount;
...
public class OrderService {
	...
	@Inject
	OrderCountService countService;

	@Inject
	services.order.OrderService orderSvc;
	...
	public Page<OdItem> getOrdersList(String email, MemberOrderForm form,
		...
		int pageCount = provider
				.searchOrderCount(email, form, siteId, isNormal);
		if (pageCount > 0) {
			...
			for (Order ord : orders) {
				...
				List<OrderPack> opack = orderPackMapper
						.getOrderPacksByOrderId(ord.getIid());
				...
				for (OrderItem oitem : items) {
					...
					og.setImgurl(oitem.getCimageurl());
	 ...
	 * @return OrderCount
	 */
	public OrderClassCount getOrderCount(String email, Integer siteId,
			Integer isShow, boolean isNormal) {

		OrderClassCount occ = new OrderClassCount();
		Integer all = orderMapper.getCountByEmailAndStatus(email, null, siteId,
				isShow, isNormal);
		Integer pending = orderMapper.getCountByEmailAndStatus(email,
				PAYMENT_PENDING, siteId, isShow, isNormal);
		Integer confirmed = orderMapper.getCountByEmailAndStatus(email,
				PAYMENT_CONFIRMED, siteId, isShow, isNormal);
		Integer cancelled = orderMapper.getCountByEmailAndStatus(email,
				ORDER_CANCELLED, siteId, isShow, isNormal);
		Integer processing = orderMapper.getCountByEmailAndStatus(email,
				PROCESSING, siteId, isShow, isNormal);
		Integer onHold = orderMapper.getCountByEmailAndStatus(email, ON_HOLD,
				siteId, isShow, isNormal);
		Integer dispatched = orderMapper.getCountByEmailAndStatus(email,
				DISPATCHED, siteId, isShow, isNormal);
		Integer completed = orderMapper.getCountByEmailAndStatus(email,
				COMPLETED, siteId, isShow, isNormal);
		Integer refunded = orderMapper.getCountByEmailAndStatus(email,
				REFUNDED, siteId, isShow, isNormal);
		Integer payin = orderMapper.getCountByEmailAndStatus(email,
				PAYMENT_PROCESSING, siteId, isShow, isNormal);
		Integer recycle = orderMapper.getCountByEmailAndStatus(email, null,
				siteId, 2, isNormal);
	 ...
	 * @return
	 */
	public OrderDetailInfo getOrderDetail(String email, String orderId,
			int langId) {
		...
		List<OrderPack> opack = orderPackMapper.getOrderPacksByOrderId(order
				.getIid());
		if (opack != null && opack.size() > 0) {
			String shipid = "";
			for (OrderPack op : opack) {
	...
	public HashMap<String, Object> completedOrder(int oiid, String email) {
		...
		Order o = orderMapper.getOrderByOrderId(oiid);
		if (o.getIstatus() != DISPATCHED) {
			objMap.put(
					"re",
					BaseResultType.UPDATE_ORDER_STATUS_NOT_DISPATCHED_ERROR_CODE);// 状态不能确认
			return objMap;
		}
		b = statusSerice.changeOrdeStatusValidEmailAndStatus(oiid,
	...
	public boolean deleteOrders(List<Integer> idList, String email) {
		...
		return updateService.updateShowValidEmail(
				OrderUpdateService.SHOW_TYPE_FALSE, idList, email);
	}
	...
	public boolean restoreOrders(List<Integer> idList, String email) {
		...
		return updateService.updateShowValidEmail(
				OrderUpdateService.SHOW_TYPE_TURE, idList, email);

	}

	public PaymentContext test(String orderId, int langID) {
		PaymentContext context = orderSvc.getPaymentContext(orderId, langID);
		return context;
	}
