...
public class ProductReviewController extends TokenController {
	...
	public Result addReview() {
		...
		if (form.hasErrors()) {
			...
			resultMap.put("msg", BaseResultType.REVIEW_ADD_FORM_GET_ERROR_MSG);
					...
					if (ValidataUtils.checkSize(fc.size(), 1000) == false) {
						return ok(Json
								.toJson(new BaseJson(
										...
										BaseResultType.UPLOAD_REVIEW_IMG_FILE_SIZE_OVER_MAX_ERROR_MSG)));
					}
				} catch (Exception e) {
					resultMap.put("re", BaseResultType.EXCEPTION);
					resultMap.put("msg", BaseResultType.EXCEPTIONMSG);
