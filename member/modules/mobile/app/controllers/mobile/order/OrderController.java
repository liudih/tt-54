...
public class OrderController extends TokenController {
	...
	public Result getOrderDetail(String oid) {
		...
		try {
			String email = loginService
					.getLoginMemberEmail(CommonDefn.ISLOGINTRUEEMAIL);
			Integer langID = mobileService.getLanguageID();
			OrderDetailInfo odtl = orderService.getOrderDetail(email, oid,
					langID);
	...
	public Result addOrder() {
		...
		String ip = "0.0.0.0";
		String vhost = "app.tomtop.com";
		if (context != null) {
			ip = mobileService.getMobileContext().getIp();
			vhost = context.getHost();
		}
		...
		if (ValidataUtils.validateLength(ao.getMsg(), 500) == false) {
			...
			objMap.put("msg", BaseResultType.MESSAGE_LENGTH_OVER_ERROR_MSG);
			return ok(Json.toJson(objMap));
		}
		if (ValidataUtils.validateNull(ao.getCartid()) == false) {
			objMap.put("re", BaseResultType.ORDER_ADD_CART_IS_NULL_ERROR_CODE);
			objMap.put("msg", BaseResultType.ORDER_ADD_CART_IS_NULL_ERROR_MSG);
			return ok(Json.toJson(objMap));
		}
		if (ao.getAddrid() == 0) {
			objMap.put("re", BaseResultType.ORDER_ADDRESS_IS_NULL_ERROR_CODE);
			objMap.put("msg", BaseResultType.ORDER_ADDRESS_IS_NULL_ERROR_MSG);
			return ok(Json.toJson(objMap));
		}
		if (ao.getShipid() == 0) {
			objMap.put("re", BaseResultType.ORDER_SHIPPING_IS_NULL_ERROR_CODE);
			objMap.put("msg", BaseResultType.ORDER_SHIPPING_IS_NULL_ERROR_MSG);
			return ok(Json.toJson(objMap));
		}
		try {
			...
			objMap.put("msg",
					BaseResultType.ADDORDERERROR + " : " + e.toString());
		...
		try {
			...
			if (StringUtils.isNotBlank(iids)) {
				...
				if (b != false) {
					...
					objMap.put("msg", BaseResultType.UPDATEERROR);
					return ok(Json.toJson(objMap));
				}
			}
	...
	public Result comfirOrder(Integer oiid) {
		...
		try {
			...
			if (re == BaseResultType.COMFIR_ORDER_ERROR_CODE) {
				...
				objMap.put("msg", BaseResultType.COMFIR_ORDER_ERROR_MSG);
				return ok(Json.toJson(objMap));
			}
	...
	public Result orderTest(String orderId) {
		PaymentContext context = orderService.test(orderId, 1);
		HashMap<String, Object> objMap = new HashMap<String, Object>();
		objMap.put("pc", context);
