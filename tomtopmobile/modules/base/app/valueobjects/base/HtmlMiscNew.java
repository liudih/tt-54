...
public class HtmlMiscNew {
	...
	private void addHead(Html html) {
		if (headRendered) {
	...
	private void addHeadOnce(Html html) {
	...
	public void addHeadScriptOnce(String src) {
		...
		for (String cell : srcs) {
			String s = cell.trim();
			if (!headString.contains(s)) {
				headString.add(s);
				Html html = Html.apply("<script type='text/javascript' src='"
						+ s + "'></script>");
	 ...
	 * @param src
	 */
	public void addHeadScriptOnce(Call src) {
		String srcStr = src.toString();
		this.addHeadScriptOnce(srcStr);
	}

	public void addHeadScriptOnce(Html src) {
		String srcStr = src.toString();
		this.addHeadScriptOnce(srcStr);
	}
	...
	public void addTailScriptOnce(String src) {
		...
		for (String cell : srcs) {
			...
			if (!tailString.contains(s)) {
				tailString.add(s);
				Html html = Html.apply("<script type='text/javascript' src='"
						+ s + "'></script>");
				this.addTailOnce(html);
			}
		}
	}

	public void addTailScriptOnce(Call src) {
		String srcStr = src.toString();
		this.addTailScriptOnce(srcStr);
	}

	public void addTailScriptOnce(Html src) {
		String srcStr = src.toString();
		this.addTailScriptOnce(srcStr);
	}
	...
	public void addHeadCssOnce(String src) {
		...
		for (String cell : srcs) {
			...
			if (!headString.contains(s)) {
				headString.add(s);
				Html html = Html.apply("<link href='" + s
	 ...
	 *            css的src
	 */
	public void addHeadCssOnce(Call src) {
		String srcStr = src.toString();
		this.addHeadCssOnce(srcStr);
	}

	public void addHeadCssOnce(Html src) {
		String srcStr = src.toString();
		this.addHeadCssOnce(srcStr);
	}
}
